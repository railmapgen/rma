...
let [filePath, inputString] = process.argv.slice(2);
filePath = filePath || "./pcfFonts/wenquanyi_12pt.pcf";
const character = inputString?.at(0) || "唐";

import { readFile } from "fs/promises";
const buffer = await readFile(filePath);
const arrayBuffer = buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength) as ArrayBuffer;
const bitmap = parsePCFAndGetGlyph(arrayBuffer, character);
if (bitmap) printMatrix(bitmap);
...
function parsePCFAndGetGlyph(
  ...
  // 5) Parse PCF_BDF_ENCODINGS => encData
...
function parseBdfEncodings(
  ...
  const count =
    (maxCharOrByte2 - minCharOrByte2 + 1) * (maxByte1 - minByte1 + 1);
